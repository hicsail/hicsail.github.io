...
$(function () {
  ...
  function newAsteroid(x, y, r) {
    var asteroid = {
      ...
      velx:
        Math.random() > 0
          ? ((Math.random() * ASTR_SPEED) / FPS) *
            (Math.random() < 0.5 ? 1 : -1)
          : ((0.5 * ASTR_SPEED) / FPS) * (Math.random() < 0.5 ? 1 : -1), //asteroid velocity
      vely:
        Math.random() > 0
          ? ((Math.random() * ASTR_SPEED) / FPS) *
            (Math.random() < 0.5 ? 1 : -1)
  ...
  function update() {
    ...
    if (ship.show && !expl) {
      if (blinkOn) {
        (ctx.strokeStyle = 'white'), (ctx.lineWidth = SHIP_SIZE / 15);
        ctx.beginPath();
        ctx.moveTo(
          //start path at nose of ship
    ...
    if (!expl) {
      ...
      //collision check
      if (ship.blinkNum == 0) {
        //check for collisions when not invincible
      ...
      if (ship.explosion == 0) {
        for (var i = 0; i < particles.length; i++) {
          //able to set the particles x,y position back to ship position
      ...
      for (var j = 1; j < vet; j++) {
        ...
        if (vet === 3) {
          ...
          if (j === 1) {
            ...
            ctx.lineTo(x + r * Math.cos(a) + r, y + r * Math.sin(a) + r);
          }
        }
